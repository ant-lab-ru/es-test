...
#
# BUILD_KEEP=1 оставляет папку сборки на месте и заводит переменные BUILD_DIR и BUILD_ELF:
# это нужно проверкам, которые смотрят не в исходники, а в собранный образ — например,
# в какую секцию компилятор положил объект. Прибирает за собой в этом случае вызывающий.
...
build_project() {
	...
	if [ -n "${BUILD_KEEP:-}" ]; then
		BUILD_DIR="$build"
		BUILD_ELF="$(ls "$build"/*.elf 2>/dev/null | head -1)"
	else
		rm -rf "$build"
	fi
	return 0
